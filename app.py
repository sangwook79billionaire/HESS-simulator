...
if st.session_state.step == 'input':
    ...
    with st.expander("📖 시뮬레이터 상세 사용 매뉴얼 (User Guide)", expanded=False):
        ...
        *   **위치 확정**: 지점을 선택하는 즉시 NASA 기상 데이터가 자동으로 연동됩니다.
        
        #### 2️⃣ Phase 2: 시장 환경 분석 (Market Intelligence)
        *   **국가 환경 리포트**: 선정된 국가의 전력 믹스, 가구별 요금 현황, 전력망(Grid) 안정성 및 고립 지역 현황을 실시간 분석하여 제공합니다.
        *   **현지 실정 확인**: 시뮬레이션 설계 전, 해당 지역의 비즈니스 환경 리포트를 검토하고 승인합니다.
        
        #### 3️⃣ Phase 3: 에너지 수요 및 소비 패턴 설계
        *   **가구 수 및 국가 데이터**: 마을 규모와 에너지 사용량을 설정합니다.
        *   **부하 믹스(Load Mix)**: 슬라이더를 조절하여 주거 중심(저녁 피크) 혹은 상업 중심(낮 피크) 패턴을 선택하세요.
        
        #### 4️⃣ Phase 4: 시나리오 분석 및 산출 로직 확인
        *   **비교 분석**: 거대 배터리(Scenario A)와 수소 하이브리드(Scenario B)의 장단점을 비교합니다.
        *   **산출 로직**: 각 항목 옆의 **ℹ️ 아이콘**에 마우스를 올리면 상세한 공학적 산출 근거를 확인할 수 있습니다.
        
        #### 5️⃣ Phase 5: 전략적 사업성 검토 (Strategic FS)
...
# --- UI: Result Step ---
    ...
    with tabs[0]:
        ...
        annual_gen_1kw = df_h['Gen_1kW'].sum()
        annual_load = total_d * 365
...
<div style='background-color: #0f172a; padding: 25px; border-radius: 16px; border: 1px solid #ff4b4b; min-height: 550px;'>
    ...
    <p style='color: #94a3b8; font-size: 13px; line-height: 1.6; margin-bottom: 25px;'>
        고발전기 효율을 반영하여 <b>연간 총 발전량과 부하의 균형</b>을 맞추는 최적 PV를 설치하고, 계절적 불균형을 대규모 배터리 뱅크로만 해소하는 시나리오입니다.
    </p>
...
"""
            ...
            with c2:
                ...
                bess_b_fixed = total_d * 1.5
                cost_bess_b_fixed = bess_b_fixed * PRICE_BESS
                # PV hybrid logic: PV for balance + margin for H2 losses (approx 20%)
                pv_hybrid_final = pv_ideal * 1.2
                capex_b_final = (pv_hybrid_final * PRICE_PV) + cost_bess_b_fixed + (el_kw * PRICE_EL) + (fc_kw * PRICE_FC) + (max(h2_stock) * 500) + (hh * 1500)
...
<div style='background-color: #0f172a; padding: 25px; border-radius: 16px; border: 1px solid #00d4ff; min-height: 550px;'>
    ...
    <p style='color: #94a3b8; font-size: 13px; line-height: 1.6; margin-bottom: 25px;'>
        고발전기 기준 PV에 <b>수소 생산 여유 캐파</b>를 더하고, 단기 대응용 1.5일 BESS와 장기 대응용 수소 시스템을 결합하여 투자 효율을 극대화한 시나리오입니다.
    </p>
    <hr style='border-color: #334155; margin: 20px 0;'>
    <ul style='list-style: none; padding: 0;'>
...
"""
            ...
            def create_net_chart(net_data, ghi_data, title):
                ...
                fig.update_layout(title=dict(text=title, font=dict(size=16)), template="plotly_dark", height=320, margin=dict(l=40, r=40, t=40, b=40), showlegend=False)
                fig.update_yaxes(title_text="순 수지 (kWh)", range=[y_min, y_max], secondary_y=False)
                fig.update_yaxes(title_text="일사량", secondary_y=True)
                return fig

            def create_storage_chart(data, col, title, color, unit):
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=data['Timestamp'], y=data[col], name=title, line=dict(color=color, width=2), fill='tozeroy'))
                fig.update_layout(title=dict(text=title, font=dict(size=16)), template="plotly_dark", height=280, margin=dict(l=40, r=40, t=40, b=40), showlegend=False)
                fig.update_yaxes(title_text=f"{unit}")
                return fig

            c_ana1, c_ana2 = st.columns(2)
            with c_ana1:
                st.markdown("<h4 style='text-align: center; color: #ff4b4b;'>Scenario A: 대용량 배터리 중심</h4>", unsafe_allow_html=True)
                st.plotly_chart(create_net_chart(monthly_net_a, monthly_ghi, "S1: 월간 에너지 수지"), use_container_width=True)
                st.plotly_chart(create_storage_chart(df_a, 'SOC_A', "배터리 충전 상태 (SOC %)", "#4CAF50", "%"), use_container_width=True)

            with c_ana2:
                st.markdown("<h4 style='text-align: center; color: #00d4ff;'>Scenario B: 수소 하이브리드</h4>", unsafe_allow_html=True)
                st.plotly_chart(create_net_chart(monthly_net_b, monthly_ghi, "S2: 월간 에너지 수지"), use_container_width=True)
                st.plotly_chart(create_storage_chart(df_h, 'H2_Stock', "수소 저장량 트렌드 (kg)", "#00d4ff", "kg"), use_container_width=True)
            # 4. CAPEX 상세 내역 및 비교
            st.markdown("---")
